...
var cnt_iter, depth, cnt_k, i, index, p1, p2, cnt, cnt_eps, limit, mod12: integer;  
a : bin_arr; fmax, fpred, ans_f, ans_x, pred_ans_f, x, stop: real; 
...
procedure ruslan(a : bin_arr; var p1, p2:integer; var fun : f_arr);
...
writeln('Write down a depth of evolution(1..): ');
readln(depth);
writeln('Write down a limit of search for the max(1..): ');
readln(limit);
writeln('Write down a stop value of the max(1..): ');
readln(stop);
{выбор мода, два разных вайл тру, но пока один, для основного режима}
cnt_k := 0; cnt_iter := 0; cnt := 1; 
sum_iter := 0; cnt_it2 := 0; cnt_eps := 0;
...
if mod12 = 1 then 
	begin
	assign(h, 'for_gena_na.txt');
	rewrite(h);
	for i := 1 to n do write(h, fun[i], ' ');
	if flag then begin
		for i := 1 to n do write(fun[i]:10:6, '  ', rebin(a[i]), ' '); writeln end;
	end;
while true do
	begin
	if cnt_iter > depth then
	...
	else if ans_f > stop then
			begin
			if mod12 = 2 then begin
				writeln('ANSWER');
				writeln(ans_x:10:6);
				writeln(ans_f:10:6);
				writeln(sum_iter + cnt_it2) end
			else if mod12 = 1 then close(h);
			break
			end
			
	else if cnt_eps > depth div 10 then
		begin
		{writeln('bad');}
		sum_iter := sum_iter + cnt_it2;
		cnt_eps := 0;
        ...
        if (cnt > limit) or (cnt_k > limit div 3) then
			begin
			if mod12 = 2 then begin
				writeln('ANSWER');
				writeln(ans_x:10:6);
			...
			if abs(fpred - fmax) < eps then cnt_eps := cnt_eps +1;
			cnt := cnt + 1;
		end
